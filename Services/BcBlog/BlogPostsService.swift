...
public struct BlogPostsService {
    ...
    public func getBlogPosts() async throws -> [BlogPost] {
    ...
    /// - Returns: 該当するブログ記事
    public func getBlogPost(id: Int) async throws -> BlogPost {
    ...
    /// - Returns: 追加したブログ記事
    public func addBlogPost(_ request: BlogPostAddRequest) async throws -> BlogPost {
    ...
    /// - Returns: 編集したブログ記事
    public func editBlogPost(id: Int, _ request: BlogPostEditRequest) async throws -> BlogPost {
        let files = request.eyeCatch.map {
            [(name: "eye_catch", data: $0.data, fileName: $0.fileName, mimeType: $0.mimeType)]
        } ?? []
    ...
    /// ブログ記事を削除する
    /// - Parameter id: ブログ記事 ID
    public func deleteBlogPost(id: Int) async throws {
