...
struct CustomTablesService {
    let client: ApiClient

    /// カスタムテーブル一覧を取得する
    /// - Returns: カスタムテーブルの配列（該当なしの場合は空配列）
    func getCustomTables() async throws -> [CustomTable] {
    ...
    func getCustomTable(id: Int) async throws -> CustomTable {
    ...
    func addCustomTable(_ request: CustomTableAddRequest) async throws -> CustomTable {
    ...
    func editCustomTable(id: Int, _ request: CustomTableEditRequest) async throws -> CustomTable {
    ...
    /// カスタムテーブルを削除する
    /// - Parameter id: カスタムテーブル ID
    func deleteCustomTable(id: Int) async throws {
